...
export default function Home() {
  ...
  const [messages, setMessages] = useState([
    {
      role: "ai",
      text: "Welcome to your personal travel planner! Where are you dreaming of going next?",
      data: null
    }
  ]);
  ...
  return (
    <div className="min-h-screen bg-[#141210] text-stone-100 font-sans selection:bg-amber-500 selection:text-black">
      
      {/* Navbar */}
      <nav className="absolute top-0 left-0 right-0 z-50 px-8 py-6 flex items-center justify-between bg-gradient-to-b from-black/80 to-transparent">
        <div className="flex items-center gap-2.5 cursor-pointer" onClick={() => setCurrentView("landing")}>
          ...
          <span className="text-xl font-serif tracking-tight font-bold text-white">Aura<span className="text-amber-400 font-sans">Travel</span></span>
        </div>
      ...
      {currentView === "landing" && (
        <div className="relative min-h-screen flex flex-col justify-between">
          ...
          {/* Destination Preview Cards at Bottom of Hero */}
          <div className="relative z-10 max-w-6xl mx-auto px-8 pb-16 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="relative group rounded-3xl overflow-hidden shadow-xl h-48 border border-white/10">
              <img src="https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=600&q=80" alt="Beach" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-5">
                <span className="text-white font-serif font-bold text-lg">Pristine Beaches</span>
              </div>
            </div>
            <div className="relative group rounded-3xl overflow-hidden shadow-xl h-48 border border-white/10">
              <img src="https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=600&q=80" alt="Mountain" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-5">
                <span className="text-white font-serif font-bold text-lg">Alpine Peaks</span>
              </div>
            </div>
            <div className="relative group rounded-3xl overflow-hidden shadow-xl h-48 border border-white/10">
              <img src="https://images.unsplash.com/photo-1548013146-72479768bada?auto=format&fit=crop&w=600&q=80" alt="Palace" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex items-end p-5">
                <span className="text-white font-serif font-bold text-lg">Historic Palaces</span>
              </div>
            </div>
          </div>
        </div>
      ...
      {currentView === "dashboard" && (
        <div className="flex-1 flex flex-col md:flex-row h-[calc(100vh-81px)] overflow-hidden bg-[#141210]">
            ...
            <div className="absolute bottom-0 w-full bg-[#141210]/90 backdrop-blur-md border-t border-stone-800 p-4">
              <div className="max-w-3xl mx-auto relative flex items-center">
                <input
                  ...
                  placeholder="Ask your travel companion, e.g., Plan a 3-day quiet getaway to Kyoto..."
                  value={input}
